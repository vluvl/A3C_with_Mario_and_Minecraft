...
class Treechop2(SimpleHumanEmbodimentEnvSpec):
    def __init__(self, *args, **kwargs):
        if 'name' not in kwargs:
            kwargs['name'] = 'MineRLTreechop-custom'
    ...
    def create_rewardables(self) -> List[Handler]:
        return [
            handlers.RewardForCollectingItems([
                ...
                dict(type="seed", amount=1, reward=1.0),
            ])
        ]

    def create_agent_start(self) -> List[Handler]:
        gui_handler = handlers.GuiScale(self.guiscale_range)
        gamma_handler = handlers.GammaSetting(self.gamma_range)
        fov_handler = handlers.FOVSetting(np.random.uniform(*self.fov_range))
        cursor_size_handler = handlers.FakeCursorSize(
            np.random.randint(self.cursor_size_range[0], self.cursor_size_range[1] + 1))
        return [                            #super().create_agent_start() +
            gui_handler, gamma_handler,fov_handler,cursor_size_handler,
            handlers.AgentStartBreakSpeedMultiplier(10000),
            handlers.SimpleInventoryAgentStart([
                dict(type="golden_axe", quantity=1)
            ])
        ]
